...
export default function PlaygroundPage() {
  ...
  return (
    <div className="playground-container">
      ...
      <div className="h-[calc(100vh-7rem)] flex flex-col gap-8">
        {/* Header with stepped progress */}
        <div className="welcome-fade-in">
          <div className="flex flex-wrap items-start justify-between gap-6">
            <div className="playground-header space-y-3">
              <div className="flex items-center gap-3">
                <div className="header-gradient flex h-14 w-14 items-center justify-center rounded-2xl shadow-xl shadow-amber-500/30">
                  <Sparkles className="h-7 w-7 text-white" />
                </div>
                <div>
                  <h1 className="text-4xl font-bold tracking-tight bg-gradient-to-r from-clay-900 to-terracotta-600 bg-clip-text text-transparent dark:from-amber-100 dark:to-amber-300">
                    Playground
                  </h1>
                  <p className="text-clay-700 dark:text-amber-200 text-sm mt-0.5 font-medium">
              ...
              <div className="flex items-center gap-2 ml-1">
                <div className={cn(
                  "step-indicator flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-semibold transition-all",
                  modelId ? "complete text-white shadow-lg" : "active text-white shadow-lg"
                )}>
                  <div className="w-1.5 h-1.5 rounded-full bg-current" />
                  {modelId ? "Model selected" : "Choose model"}
                </div>
                ...
                {modelId && (
                  <div className={cn(
                    ...
                    <div className="w-1.5 h-1.5 rounded-full bg-current" />
                    {messages.length > 0 ? "Chatting" : "Start chatting"}
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="flex gap-3">
              <Button
                ...
                className="gap-2 border-amber-300 bg-white/80 hover:bg-amber-50 text-clay-800 font-medium"
              >
              ...
              <Button
                ...
                size="sm"
                asChild
                className="gap-2 border-terracotta-500 bg-terracotta-500 hover:bg-terracotta-600 text-white font-medium shadow-md"
              >
        ...
        <div className="flex-1 min-h-0 flex flex-col gap-6">
            ...
            <Card className="model-card-pulse shadow-xl">
              <CardHeader className="pb-4">
                <div className="flex items-start gap-4">
                  <div className="header-gradient flex h-12 w-12 items-center justify-center rounded-xl shadow-lg shadow-amber-500/30">
                    <Zap className="h-6 w-6 text-white" />
                  </div>
                  <div className="flex-1">
                    <CardTitle className="text-2xl font-bold text-clay-900 dark:text-amber-100">
                      Choose Your AI Model
                    </CardTitle>
                    <CardDescription className="mt-1.5 text-clay-700 dark:text-amber-200">
          ...
          {modelId && (
            <div className="chat-interface grid flex-1 grid-cols-1 gap-6 lg:grid-cols-[1fr_340px] min-h-0">
              {/* Chat area */}
              <Card className="warm-card flex flex-col min-h-0 border-amber-200 dark:border-amber-800 shadow-lg">
                <CardHeader className="border-b border-amber-200 bg-gradient-to-r from-peach-100 to-warm-100 dark:from-amber-900/30 dark:to-terracotta-900/30">
                  <div className="flex items-center justify-between">
                    <div>
                      <CardTitle className="text-lg font-bold text-clay-900 dark:text-amber-100">Conversation</CardTitle>
                      <CardDescription className="text-xs mt-0.5 text-clay-700 dark:text-amber-200">
                        {messages.length === 0
                    ...
                    {messages.length > 0 && (
                      <Button
                        ...
                        className="text-xs h-8 text-terracotta-600 hover:text-terracotta-700 hover:bg-amber-100"
                      >
                        Clear chat
                      </Button>
                    )}
                ...
                {messages.length > 0 && (
                  <Card className="stat-card shadow-lg">
                    <CardHeader className="pb-3 border-b border-amber-200">
                      <CardTitle className="text-sm font-bold text-clay-900 dark:text-amber-100">Session Metrics</CardTitle>
                      <CardDescription className="text-xs text-clay-700 dark:text-amber-200">Real-time usage tracking</CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-3 pt-4">
                      <div className="grid grid-cols-2 gap-3">
                        <div className="space-y-1 bg-white/60 dark:bg-clay-900/20 p-3 rounded-lg border border-amber-200/50">
                          <p className="text-[10px] uppercase tracking-wide text-terracotta-600 dark:text-amber-400 font-bold">Input</p>
                          <p className="text-xl font-bold text-clay-900 dark:text-amber-100">
                            {(totalInputTokens / 1000).toFixed(1)}K
                          </p>
                        </div>
                        <div className="space-y-1 bg-white/60 dark:bg-clay-900/20 p-3 rounded-lg border border-amber-200/50">
                          <p className="text-[10px] uppercase tracking-wide text-terracotta-600 dark:text-amber-400 font-bold">Output</p>
                          <p className="text-xl font-bold text-clay-900 dark:text-amber-100">
